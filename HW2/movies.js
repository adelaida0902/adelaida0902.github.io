...
const movies = [
    ...
    },
    {
      ...
      popularity: 393.456,
      poster_path:
        "https://image.tmdb.org/t/p/w500/mzj2j2syZFAhCwbwT9a2vK8lzmN.jpg",
    ...
    },
    {
      ...
      popularity: 393.401,
      poster_path:
        "https://image.tmdb.org/t/p/w500/gPUihZBrsyhnoy8jc73OaFXN5qY.jpg",
...
function displayMovies(movieArray) {
    ...
    movieArray.forEach(({ id, title, overview, release_date, popularity, poster_path }) => {
        const movieList = `
            <div class="movie-container">
                ...
                <button id="add-${id}">Add to favorites❤️</button>
            </div>`;
        
        movieContainer.insertAdjacentHTML("beforeend", movieList);
        
        const addButton = document.getElementById(`add-${id}`);
        addButton.addEventListener("click", () => addToFavorites({ id, title, overview, release_date, poster_path }));
    });
}
...
function addToFavorites(movie) {
    ...
    if (document.querySelector(`#fav-${id}`)) {
        alert("This movie is already in your favorites!");
        return;
    }

    const favoriteMovie = `
        <div class="favorite-container" id="fav-${id}">
            <p class="title">${title}</p>
            <button id="remove-${id}">Remove from favorites❌</button>
        </div>`;

    favoriteList.insertAdjacentHTML("beforeend", favoriteMovie);
...
function removeFromFavorites(id) {
    const favoriteMovie = document.querySelector(`#fav-${id}`);
    if (favoriteMovie) {
